 ...
 */
export const parseSize = (size, defaultValue) => {
    if (!size)
        return defaultValue;
 ...
 */
export const parseShadow = (shadow) => {
    if (!shadow)
        return "";
...
const parseSingleShadow = (shadow) => {
 ...
 */
export const getDrawerVariants = (direction, animationType) => {
    const baseVariants = {
        slide: {
            hidden: {
 ...
 */
export const getDrawerStyle = (direction, width, height, marginLeft, marginRight, marginTop, marginBottom, padding, margin, border, shadow, opacity, backdropBlur, backgroundColor) => {
    const baseStyle = {
        position: "fixed",
        zIndex: 1000,
